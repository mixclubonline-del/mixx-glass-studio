...
import PlaceholderVisualizer from "../components/plugins/PlaceholderVisualizer";
import { getMixxAuraEngine } from "./MixxAuraEngine";
import { getPrimeEQEngine } from "./PrimeEQEngine";
import { getMixxPolishEngine } from "./MixxPolishEngine";
...
const COMPONENT_REGISTRY: Partial<Record<PluginId, React.FC<any>>> = {
  ...
  // Plugins with engines but placeholder visualizers
  "mixx-aura": PlaceholderVisualizer,
  "prime-eq": PlaceholderVisualizer,
  "mixx-polish": PlaceholderVisualizer,
  // Plugins without engines yet - using placeholders
  "mixx-morph": PlaceholderVisualizer,
  "prime-brain-stem": PlaceholderVisualizer,
  "mixx-balance": PlaceholderVisualizer,
  "mixx-ceiling": PlaceholderVisualizer,
  "prime-master-eq": PlaceholderVisualizer,
  "mixx-dither": PlaceholderVisualizer,
  "mixx-soul": PlaceholderVisualizer,
  "mixx-motion": PlaceholderVisualizer,
  "prime-lens": PlaceholderVisualizer,
  "mixx-brainwave": PlaceholderVisualizer,
  "mixx-spirit": PlaceholderVisualizer,
  "mixx-analyzer-pro": PlaceholderVisualizer,
  "prime-router": PlaceholderVisualizer,
  "mixx-port": PlaceholderVisualizer,
  "telemetry-collector": PlaceholderVisualizer,
  "prime-bot-console": PlaceholderVisualizer,
};
...
const resolveEngineFactory = (
  ...
  switch (id) {
    ...
    // Plugins with engines
    case "mixx-aura":
      return () => getMixxAuraEngine(ctx);
    case "prime-eq":
      return () => getPrimeEQEngine(ctx);
    case "mixx-polish":
      return () => getMixxPolishEngine(ctx);
    // All other plugins use placeholder engine
    default:
...
export function getPluginRegistry(ctx: BaseAudioContext): PluginConfig[] {
  // All plugins should now be registered - this check is for development debugging
  if (import.meta.env.DEV) {
    const missingComponents = Object.values(PLUGIN_CATALOG)
      .filter((entry) => !COMPONENT_REGISTRY[entry.id]);
    
    if (missingComponents.length > 0) {
      // Log once per session to avoid console spam
      const warningKey = 'plugin-registry-warnings-shown-v2';
      try {
        if (!sessionStorage.getItem(warningKey)) {
          missingComponents.forEach((entry) => {
            console.warn(
              `[PLUGIN] Plugin "${entry.id}" (${entry.name}) is in catalog but has no component. It will not be available in the plugin browser. Add it to COMPONENT_REGISTRY to enable it.`
            );
          });
