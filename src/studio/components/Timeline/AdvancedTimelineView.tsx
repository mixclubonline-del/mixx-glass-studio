...
export const AdvancedTimelineView: React.FC<AdvancedTimelineViewProps> = ({
  ...
  useTimelineKeyboardShortcuts(onSeek, undefined,
  // play/pause handled by parent
  undefined,
  ...
  const [chopModeRegion, setChopModeRegion] = useState<{
    regionId: string;
    buffer: AudioBuffer;
  } | null>(null);
  ...
  const handleConvertSlicesToPattern = (slicePoints: number[], velocities?: number[]) => {
    ...
    const pattern = createPatternFromSelection(`${region.name} Chops`, 'drums', sliceRegionIds);
    if (pattern) {
  ...
  return <div className="h-full flex flex-col relative" style={{
    background: `
          var(--gradient-mesh),
      ...
      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/3 left-1/4 w-[500px] h-[500px] rounded-full" style={{
        background: 'radial-gradient(circle, hsl(275 100% 65% / 0.08) 0%, transparent 70%)',
        filter: 'blur(120px)',
        animation: 'float 25s ease-in-out infinite'
      }} />
        <div className="absolute bottom-1/3 right-1/3 w-[400px] h-[400px] rounded-full" style={{
      ...
      <div className="flex-none flex items-center justify-between px-4 border-b border-border/30 bg-background/80" style={{
      height: '48px'
    }}>
        {/* Left section - Tools */}
        <div className="flex items-center gap-2">
          ...
          <div className="flex gap-1 p-1 rounded glass-medium border border-border/30">
            <Button size="sm" variant={timelineMode === 'audio' ? 'secondary' : 'ghost'} onClick={() => setTimelineMode('audio')} className="h-7 px-3">
              <Music2 className="h-3 w-3 mr-1" />
              Audio
            </Button>
            <Button size="sm" variant={timelineMode === 'pattern' ? 'secondary' : 'ghost'} onClick={() => setTimelineMode('pattern')} className="h-7 px-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" variant={snapMode !== 'off' ? 'secondary' : 'ghost'} onClick={() => setSnapMode(snapMode === 'off' ? 'grid' : 'off')} title={`Snap: ${snapMode}`}>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {timelineMode === 'pattern' && !patternBrowserCollapsed && <div className="flex-shrink-0" style={{
        width: `${PANEL_WIDTH_SM}px`
      }}>
            <PatternBrowser />
          </div>}
        
        {timelineMode === 'pattern' && patternBrowserCollapsed && <div className="flex-shrink-0 glass-light border-r border-gradient flex items-start justify-center pt-2" style={{
        width: `${TRACK_LIST_COLLAPSED}px`
      }}>
            <Button size="sm" variant="ghost" onClick={() => setPatternBrowserCollapsed(false)} className="h-8 w-8 p-0">
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-w-0">
            ...
            {showTimelineRuler && <div className="relative glass-light border-b border-gradient" style={{
          height: `${RULER_HEIGHT}px`,
          backdropFilter: 'blur(40px) saturate(180%)'
        }}>
                <TimelineRuler width={containerRef.current?.clientWidth || 800} height={RULER_HEIGHT} bpm={120} onSeek={onSeek} />
              </div>}
          ...
          minHeight: 0
            ...
            width: `${totalWidth}px`
                ...
                {/* Grid overlay */}
                <GridOverlay width={totalWidth} height={tracks.length * TRACK_HEIGHT} bpm={120} />
                
                {/* Snap guides */}
                <SnapGuide snapPositions={snapGuidePositions} zoom={zoom} height={tracks.length * TRACK_HEIGHT} />
                
                {/* Playhead */}
                <Playhead containerWidth={containerRef.current?.clientWidth || 800} containerHeight={tracks.length * TRACK_HEIGHT} />

                {/* Crossfade Renderer */}
                <CrossfadeRenderer regions={regions} zoom={zoom} trackHeight={100} />
                
                {/* Tracks */}
                {tracks.map(track => <TimelineTrackRow key={track.id} track={track} regions={getTrackRegions(track.id)} audioBuffers={audioBuffers} zoom={zoom} isSelected={selectedTrackId === track.id} onSelectTrack={handleSelectTrack} onUpdateRegion={updateRegion} onSelectRegion={handleSelectRegion} onSplitRegion={handleSplitRegion} selectedRegionIds={new Set(selectedRegionIds)} onChopSample={handleChopSample} />)}
              ...
              return trackPatterns.map(instance => <div key={instance.id} className="absolute" style={{
                ...
                pointerEvents: 'none'
                      ...
                      <PatternInstance instance={instance} zoom={zoom} scrollX={scrollX} trackHeight={TRACK_HEIGHT} isSelected={selectedRegionIds.includes(instance.id)} onSelect={handleSelectRegion} />
                    </div>);
            })}
                
                {tracks.length === 0 && <div className="absolute inset-0 flex items-center justify-center">
          ...
          {!trackListCollapsed && <div className="flex-shrink-0 border-l border-gradient flex flex-col" style={{
        ...
        boxShadow: 'inset -1px 0 0 rgba(255, 255, 255, 0.1)'
              ...
              {showTimelineRuler ? <div className="flex items-center justify-start px-4 glass-ultra border-b border-gradient" style={{
          height: `${RULER_HEIGHT}px`
        }}>
                  <Button size="sm" variant="ghost" onClick={() => setTrackListCollapsed(true)} className="h-8 w-8 p-0 micro-interact chromatic-hover">
                    <ChevronRight size={16} />
                  </Button>
                </div> : <div className="flex items-center justify-start px-4 glass-ultra border-b border-gradient py-2">
                  <Button size="sm" variant="ghost" onClick={() => setTrackListCollapsed(true)} className="h-8 w-8 p-0 micro-interact chromatic-hover">
                    <ChevronRight size={16} />
                  </Button>
                </div>}
              ...
              <div className="flex-1 overflow-y-auto">
                {tracks.map(track => <ProfessionalTrackHeader key={track.id} id={track.id} name={track.name} color={track.color} muted={track.muted} solo={track.solo} recordArmed={track.recordArmed} locked={track.locked} isSelected={selectedTrackId === track.id} height={TRACK_HEIGHT} onSelect={handleSelectTrack} onMuteToggle={handleMuteToggle} onSoloToggle={handleSoloToggle} onRecordArmToggle={handleRecordArmToggle} onLockToggle={handleLockToggle} />)}
              </div>
            </div>}
          
          {/* Collapsed Track List Toggle Button - STANDARD COLLAPSED WIDTH */}
          {trackListCollapsed && <div className="flex-shrink-0 glass-light border-l border-gradient flex items-start justify-center pt-2" style={{
        width: `${TRACK_LIST_COLLAPSED}px`,
        backdropFilter: 'blur(60px) saturate(200%)'
      }}>
              <Button size="sm" variant="ghost" onClick={() => setTrackListCollapsed(false)} className="h-8 w-8 p-0 micro-interact chromatic-hover">
                <ChevronLeft size={16} />
              </Button>
            </div>}

          {/* Right Browser Panel */}
          <ArrangeBrowserPanel selectedTrackId={selectedTrackId} onFileSelect={onFileSelect} onPluginSelect={onPluginSelect} isCollapsed={browserCollapsed} onToggleCollapse={() => setBrowserCollapsed(!browserCollapsed)} onOpenPianoRoll={setPianoRollRegionId} />
        </div>

      {/* Piano Roll Overlay */}
      {pianoRollRegionId && <PianoRollOverlay regionId={pianoRollRegionId} onClose={() => setPianoRollRegionId(null)} />}

      {/* Sample Chop Mode Overlay */}
      {chopModeRegion && (() => {
      const region = regions.find(r => r.id === chopModeRegion.regionId);
      if (!region) return null;
      return <SampleChopMode region={region} audioBuffer={chopModeRegion.buffer} zoom={zoom} scrollX={scrollX} onClose={() => setChopModeRegion(null)} onCreateSlices={handleCreateSlices} onConvertToPattern={handleConvertSlicesToPattern} />;
    })()}

      {/* Add Track Dialog */}
      <AddTrackDialog open={addTrackDialogOpen} onOpenChange={setAddTrackDialogOpen} onCreateTrack={handleCreateTrack} />
    </div>;
};