...
const FlowRuntime: React.FC<FlowRuntimeProps> = ({ arrangeFocusToken }) => {
  ...
  const handlePlayPause = useCallback(async () => {
    ...
    if (newIsPlaying && ctx.state === 'suspended') {
      try {
        await ctx.resume();
        console.log('[AUDIO] AudioContext resumed on first play');
      } catch (error) {
    ...
    // CRITICAL: Verify audio output connection is active
    if (newIsPlaying && masterNodesRef.current && translationMatrixRef.current) {
      const masterOutput = masterNodesRef.current.output as GainNode;
      const masterInput = masterNodesRef.current.input as GainNode;
      const tm = translationMatrixRef.current;
      
      // Ensure master input gain is not zero (tracks connect here)
      if (masterInput && masterInput.gain.value === 0) {
        console.warn('[AUDIO] ⚠️ Master input gain is 0 - setting to 0.85 (default)');
        masterInput.gain.value = 0.85;
      }
      
      // Ensure master output gain is not zero
      if (masterOutput.gain.value === 0) {
        console.warn('[AUDIO] ⚠️ Master output gain is 0 - setting to 1');
        masterOutput.gain.value = 1;
      }
      
      // Verify TranslationMatrix is attached
      if (!tm.attached) {
        console.warn('[AUDIO] ⚠️ TranslationMatrix not attached - reattaching...');
        tm.attach(masterOutput, ctx.destination);
        tm.activate(translationProfileRef.current);
      }
      
      // Verify TranslationMatrix dryGain is active (for flat profile)
      const dryGain = (tm as any).dryGain as GainNode;
      if (dryGain && dryGain.gain.value === 0 && tm.getActiveProfile() === 'flat') {
        console.warn('[AUDIO] ⚠️ TranslationMatrix dryGain is 0 - setting to 1');
        dryGain.gain.value = 1;
      }
      
      console.log('[AUDIO] Connection verified:', {
        masterInputGain: masterInput?.gain?.value ?? 'N/A',
        masterOutputGain: masterOutput.gain.value,
        tmAttached: tm.attached,
        tmProfile: tm.getActiveProfile(),
        dryGain: dryGain?.gain?.value ?? 'N/A',
        ctxState: ctx.state,
      });
    } else if (newIsPlaying && masterNodesRef.current && nativeAudioStreamNodeRef.current) {
      // Native audio path
      const masterOutput = masterNodesRef.current.output as GainNode;
      const masterInput = masterNodesRef.current.input as GainNode;
      
      if (masterInput && masterInput.gain.value === 0) {
        console.warn('[AUDIO] ⚠️ Master input gain is 0 - setting to 0.85');
        masterInput.gain.value = 0.85;
      }
      ...
      console.log('[AUDIO] Native audio path active:', {
        masterInputGain: masterInput?.gain?.value ?? 'N/A',
        masterOutputGain: masterOutput.gain.value,
        ctxState: ctx.state,
      });
    } else if (newIsPlaying && masterNodesRef.current) {
      console.error('[AUDIO] ❌ No audio output connection found!');
      // Emergency fallback: direct connection
      console.warn('[AUDIO] Creating emergency direct connection to destination');
      try {
        masterNodesRef.current.output.disconnect();
      } catch {}
      masterNodesRef.current.output.connect(ctx.destination);
      console.log('[AUDIO] ✅ Emergency direct connection established');
    }
    
    // Run comprehensive audio output diagnostics
    if (newIsPlaying && (import.meta as any).env?.DEV) {
  ...
  useEffect(() => {
    ...
    const setupAudio = async () => {
        ...
        if (isTauri) {
          ...
          // CRITICAL: Verify the connection path is actually established
          try {
            // Check if master output is connected to TranslationMatrix input
            const masterOutput = masterNodesRef.current.output as GainNode;
            console.log('[AUDIO] Master output gain:', masterOutput.gain.value);
            
            // Verify TranslationMatrix output is connected to destination
            const tmOutput = (translationMatrix as any).output as GainNode;
            if (tmOutput) {
              console.log('[AUDIO] TranslationMatrix output gain:', tmOutput.gain.value);
              console.log('[AUDIO] TranslationMatrix dryGain:', (translationMatrix as any).dryGain?.gain?.value ?? 'N/A');
            }
            
            // Ensure master gain is not zero
            if (masterOutput.gain.value === 0) {
              console.warn('[AUDIO] ⚠️ Master gain is 0 - setting to 1 for audio output');
              masterOutput.gain.value = 1;
            }
          } catch (error) {
            console.error('[AUDIO] Error verifying connection:', error);
          }
        }
        ...
        console.log('[AUDIO] Final master output connection check:', {
          ...
          masterGain: (masterNodesRef.current.output as GainNode)?.gain?.value ?? 'N/A',
        });
