...
// FIX: Changed WebMidi.MIDIMessageEvent to MIDIMessageEvent to use globally available DOM types.
export type MidiMessageCallback = (message: MIDIMessageEvent) => void;

export const useMidi = () => {
  const [inputs, setInputs] = useState<MidiInput[]>([]);
  const [selectedInputId, setSelectedInputId] = useState<string | null>(null);
  // FIX: Changed WebMidi.MIDIAccess to MIDIAccess to use globally available DOM types.
