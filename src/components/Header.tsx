    ...
    if (!analysisResult || velvetScore === null) {
        return (
            <div className="flex-grow flex items-center justify-center h-full">
                <p className="text-ink/40 text-lg font-mono tracking-widest animate-pulse">
    ...
    return (
        <div className={`relative flex-grow flex items-center justify-center h-full ${animationClass}`}
            ...
            <div className="absolute w-48 h-48 transition-all duration-500 ease-out group" style={getAnchorStyle(analysisResult.body)}>
                <div className="w-full h-full rounded-full border-2 border-red-400/60"></div>
                <span className="absolute -top-4 left-1/2 -translate-x-1/2 text-xs text-red-300 uppercase opacity-0 group-hover:opacity-100 transition-opacity">Body</span>
            </div>
            <div className="absolute w-40 h-40 transition-all duration-500 ease-out delay-75 group" style={getAnchorStyle(analysisResult.soul)}>
                <div className="w-full h-full rounded-full border-2 border-fuchsia-400/60"></div>
                <span className="absolute top-1/2 -right-4 translate-x-full -translate-y-1/2 text-xs text-fuchsia-300 uppercase opacity-0 group-hover:opacity-100 transition-opacity">Soul</span>
            </div>
            <div className="absolute w-32 h-32 transition-all duration-500 ease-out delay-150 group" style={getAnchorStyle(analysisResult.air)}>
                <div className="w-full h-full rounded-full border-2 border-cyan-400/60"></div>
                 <span className="absolute -bottom-4 left-1/2 -translate-x-1/2 text-xs text-cyan-300 uppercase opacity-0 group-hover:opacity-100 transition-opacity">Air</span>
            </div>
             <div className="absolute w-24 h-24 transition-all duration-500 ease-out delay-200 group" style={getAnchorStyle(analysisResult.silk)}>
                <div className="w-full h-full rounded-full border-2 border-violet-300/60"></div>
                <span className="absolute top-1/2 -left-4 -translate-x-full -translate-y-1/2 text-xs text-violet-200 uppercase opacity-0 group-hover:opacity-100 transition-opacity">Silk</span>
            </div>
            ...
            <div className={`relative w-28 h-28 rounded-full bg-gradient-to-br ${gradient} flex flex-col items-center justify-center cursor-help group`}>
                 <span className="text-xs font-semibold uppercase tracking-wider text-ink/80">{label}</span>
                 <p className="text-4xl font-bold text-white drop-shadow-lg">{velvetScore}</p>
                 <div className="absolute left-1/2 -translate-x-1/2 top-full mt-2 p-2 w-48 bg-[rgba(10,18,34,0.9)] text-ink text-xs rounded-md shadow-lg backdrop-blur-sm border border-glass-border opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-opacity duration-200 z-50 pointer-events-none">
...
const Header: React.FC<HeaderProps> = ({ analysisResult, hushFeedback, isPlaying }) => {
  ...
  return (
    <header className="absolute top-0 left-0 right-0 z-30 p-4 h-20 flex justify-center items-center text-sm backdrop-blur-xl bg-glass-surface/95 border-b border-glass-border shadow-[0px_32px_55px_rgba(4,12,26,0.48)] pointer-events-none text-ink">
